...
def spell(spell_inputs):
    ...
    xcol_string=x_col+":O"
    if st.checkbox("Show as continuous?",key="time_bar_x_is_cont"):
        xcol_string=x_col+":Q"     
    y_col = st.selectbox("Select y axis for bar chart", mana.columns)
    ycol_string=alt.Y(y_col)
    if st.checkbox("Show as sorted?",key="time_bar_sort_y"):
    ...
    chart = (
        ...
        .properties(
            title="Bar graph of " + x_col + "," + y_col + " at " + str(curr_time)
        )
    ...
    if st.button("Animate Graph"):
        ...
        for times in sorted_vals:
            ...
            curr_chart = (
                ...
                .properties(
                    title="Bar graph of " + x_col + "," + y_col + " at " + str(times)
                )
                .configure_title(fontSize=20,)
                .configure_axis(labelFontSize=20, titleFontSize=20)
                .configure_legend(labelFontSize=20, titleFontSize=20)
